...
datef = input(put(intnx('month',today(),0,'begin'),date9. ),$10.);
Call symput("periodo", date0);
Call symput("fechad", dated);
Call symput("fechaf", datef);
RUN;
%put &periodo; /*periodo anterior*/
%put &fechad;/*fecha inicio anterior ok trx-pagos tda*/
%put &fechaf;/*fecha fin anterior ok trx-pagos tda*/
