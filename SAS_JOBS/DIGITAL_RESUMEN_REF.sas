...
ultimo_dia_mes= input(put(intnx('month',today(),0,'end'),day.),best16.);
...
%DELETE_BULK(dgtl_resumen_ref,raw,oracloud,0);

%INCLUDE"/sasdata/users_BI/RESULTADOS/oradiag_user_bi/AWS/AWS_EXPORT.sas";
%EXPORTACION(dgtl_resumen_ref,&libreria..digital_resumen_ref,raw,oracloud,0);
...
SUBJECT = ("MAIL_AUTOM: Proceso DIGITAL_RESUMEN_REF");
FILE OUTBOX;
 PUT "Estimados:";
 PUT "		Proceso DIGITAL_RESUMEN_REF, ejecutado con fecha: &fechaeDVN";
 PUT ;
 PUT ;
 PUT ;
 PUT 'Proceso Vers. 04'; 
