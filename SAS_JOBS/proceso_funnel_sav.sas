...
/* 2022-07-13 -- V03 -- Sergio J. -- Se agrega código de exportación para alimentar a Tableau

/* 2021-07-05 -- V02 -- Sergio J. -- Se cambia de libreria de cnavar a rsepulv

/* 2021-02-25 -- V01 -- José A.   -- Versión Original
/* INFORMACIÓN:
...
DATA _null_;
V_inicioActual  = input(put(intnx('month',today(),0,'begin' ),Date9.  ),$10.); /*cambiar 0 a -1 para ver cierre mes anterior*/
...
dateyy	= input(put(intnx('month',today(),-3,'end'	),yymmn6.   ),$10.); /*cambiar 0 a -1 para ver cierre mes Actual*/
...
PROC SQL;
   ...
   SELECT t1.*,
          ...
          t2.NOMB_ORIGEN, 
          t2.NOMB_TIPO
      FROM PUBLICIN.TABLON_VISITAS_&fechax t1  /* PASAR ESTA TABLE DE PETER A lib. ( DAVID)*/
...
inicioAntes  = input(put(intnx('month',today(),0,'begin' ),yymmdd10.  ),$10.); /*cambiar 0 a -1 para ver cierre mes anterior*/
terminoAntes = input(put(intnx('month',today()-0,-0,'end' ),yymmdd10.  ),$10.); /*cambiar 0 a -1 para ver cierre mes anterior*/
...
DATA _null_;
V_inicioAntes  = input(put(intnx('month',today(),-1,'begin' ),Date9.  ),$10.); /*cambiar 0 a -1 para ver cierre mes anterior*/
V_terminoAntes = input(put(intnx('month',today()-1,-1,'same' ),Date9.  ),$10.); /*cambiar 0 a -1 para ver cierre mes anterior*/
datey	= input(put(intnx('month',today(),-1,'end'	),yymmn6.   ),$10.); /*cambiar 0 a -1 para ver cierre mes anterior*/
dateyy	= input(put(intnx('month',today(),-3,'end'	),yymmn6.   ),$10.); /*cambiar 0 a -1 para ver cierre mes anterior*/
exec	= compress(input(put(today(),yymmdd10.),$10.),"-",c);
exec3	= compress(input(put(intnx('month',today(),-1,'begin' ),yymmdd10.  ),$10.),"-",c);
exec4	= compress(input(put(intnx('month',today(),-1,'same' ),yymmdd10.  ),$10.),"-",c);
...
terminoAntes = input(put(intnx('month',today()-1,-1,'same' ),yymmdd10.  ),$10.); /*cambiar 0 a -1 para ver cierre mes anterior*/
datey	= input(put(intnx('month',today(),-1,'end'	),yymmn6.   ),$10.); /*cambiar 0 a -1 para ver cierre mes anterior*/
exec	= compress(input(put(today(),yymmdd10.),$10.),"-",c);
...
DATA _null_;
V_inicioAntes_12  = input(put(intnx('month',today(),-12,'begin' ),Date9.  ),$10.); /*cambiar 0 a -1 para ver cierre mes anterior*/
V_terminoAntes_12 = input(put(intnx('month',today(),-12,'same' ),Date9.  ),$10.); /*cambiar 0 a -1 para ver cierre mes anterior*/
datez	= input(put(intnx('month',today(),-12,'end'	),yymmn6.   ),$10.); /*cambiar 0 a -1 para ver cierre mes anterior*/
datezz	= input(put(intnx('month',today(),-14,'end'	),yymmn6.   ),$10.); /*cambiar 0 a -1 para ver cierre mes anterior*/
exec	= compress(input(put(today(),yymmdd10.),$10.),"-",c);
exec5	= compress(input(put(intnx('month',today(),-12,'begin' ),yymmdd10.  ),$10.),"-",c);
exec6	= compress(input(put(intnx('month',today(),-12,'same' ),yymmdd10.  ),$10.),"-",c);
...
inicioAntes_12  = input(put(intnx('month',today(),-12,'begin' ),yymmdd10.  ),$10.); /*cambiar 0 a -1 para ver cierre mes anterior*/
terminoAntes_12 = input(put(intnx('month',today(),-12,'same' ),yymmdd10.  ),$10.); /*cambiar 0 a -1 para ver cierre mes anterior*/
datez	= input(put(intnx('month',today(),-12,'end'	),yymmn6.   ),$10.); /*cambiar 0 a -1 para ver cierre mes anterior*/
exec	= compress(input(put(today(),yymmdd10.),$10.),"-",c);
...
PROC SQL;
CREATE TABLE  &libreria..FUNNEL_SAV_RESUMEN_UNIF AS
...
,MOVIL_MOVIL_&fechay
,MOVIL_MOVIL_&fechaz
,FUNNEL_NEW_SAV_&fechax
,FUNNEL_NEW_SAV_&fechay
,FUNNEL_NEW_SAV_&fechaz
,FUNNEL_NEW_SAV_F_&fechax
,FUNNEL_NEW_SAV_F_&fechay
,FUNNEL_NEW_SAV_F_&fechaz
,FUNNEL_NEW_RESUMEN_&fechax
,FUNNEL_NEW_RESUMEN_&fechay
,FUNNEL_NEW_RESUMEN_&fechaz
,FUNNEL_SAV_RESUMEN_&fechax
,FUNNEL_SAV_RESUMEN_&fechay
,FUNNEL_SAV_RESUMEN_&fechaz
;
QUIT;
...
%DELETE_BULK(ppff_funnel_sav_res_unif,raw,oracloud,0);
%INCLUDE"/sasdata/users_BI/RESULTADOS/oradiag_user_bi/AWS/AWS_EXPORT.sas";
%EXPORTACION(ppff_funnel_sav_res_unif,&libreria..funnel_sav_resumen_unif,raw,oracloud,0);
...
SELECT EMAIL into :DEST_6
	FROM result.EDP_BI_DESTINATARIOS WHERE CODIGO = 'PPFF_PM_AVANCE';
...
FROM = ("&EDP_BI")
TO = ("&DEST_2","&DEST_3","&DEST_4","&DEST_5","&DEST_6")
CC = ("&DEST_1")
