...
/* export data csv */
proc export data=&libreria..resumen_kpi_internet
outfile="/sasdata/users94/user_bi/ORACLOUD/RESUMEN_KPI_INTERNET.csv"
dbms=dlm
replace;
delimiter="|";
quit;
...
%put #######################################################################;
%put ####       AGREGAMOS LA TABLA PARA LECTURA CON TABLEAU             ####;
%put #######################################################################;

LIBNAME ORACLOUD ORACLE sql_functions=all READBUFF=1000 INSERTBUFF=1000 PATH="REPORTSAS.WORLD" SCHEMA=SAS_ADM authdomain=DBOracleCloud_Auth;

%if (%sysfunc(exist(oracloud.nverdejo_resumen_kpi_internet ))) %then %do;

%end;
%else %do;
proc sql noprint;
connect using oracloud;
create table oracloud.nverdejo_resumen_kpi_internet
(fecha date, 
canal  varchar, 
logueos_acumulados num, 
oferta_sav num, 
 oferta_av num, 
oferta_CONSUMO num, 
oferta_av_acumulada num, 
oferta_sav_acumulada num, 
oferta_cons_acumulada num, 
MONTO_AV num, 
TRX_AV num, 
MONTO_SAV num, 
TRX_SAV num, 
...
OFTA_SAV_DIARIA num, 
OFTA_AV_DIARIA num, 
OFTA_CONSUMO_DIARIA num, 
SIMULACIONES_AV num, 
SIMULACIONES_SAV num, 
SIMULACIONES_CONS num, 
SIMS_SAV_DIARIAS num, 
SIMS_AV_DIARIAS num, 
SIMS_CONS_DIARIAS num, 
Seguros_Av num, 
Seguros_Sav num, 
CAPTAS_TAM num, 
CAPTAS_TR num, 
...
proc sql noprint;
connect using oracloud;
execute by oracloud ( delete from nverdejo_resumen_kpi_internet );
...
proc sql noprint;
connect using oracloud;
insert into oracloud.nverdejo_resumen_kpi_internet (
...
CAPTAS_VISTA, 
Seguros_consumo
FROM &libreria..resumen_kpi_internet;
disconnect from oracloud ;run;
...
DBMS=xlsx 
OUTFILE= "/sasdata/users94/pmunoz/RESUMEN_KPI_INTERNET_X_v4.xlsx"
  replace
;RUN;
...
SUBJECT="MAIL_AUTOM: Actualización pmunoz.RESUMEN_KPI_INTERNET_X  %sysfunc(date(),yymmdd10.)"
...
TO = ("pmunozc@bancoripley.com","rarcosm@bancoripley.com","tpiwonkas@bancoripley.com")
 CC      = ("nverdejog@bancoripley.com")
	attach =("/sasdata/users94/pmunoz/RESUMEN_KPI_INTERNET_X_v4.xlsx" content_type="excel") 
	  Type    = 'Text/Plain';
