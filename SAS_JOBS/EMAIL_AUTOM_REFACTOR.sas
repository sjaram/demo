...
/* 2023-06-07 ---- V35 -- Sergio J. -- Modificación de PERIODOS fuente del feedback email.
/* 2023-04-25 ---- V34 -- Sergio J. -- Modificación de fuente del feedback email.
/* 2023-04-06 ---- V33 -- Andrea S. -- Ajustes por error en ejecución del proceso completo.
/* 2023-03-16 ---- V32 -- Andrea S. -- Se habilita union con fuente de datos desde base QSC NEW (admisión)
									-- Se agregan correos obtenidos desde las simulaciones realizadas en sitio público como fuente de datos (descarga de datos mensual)
									-- Se agrega consulta a base de simulaciones históricas (3 meses) realizadas a través de Sitio público.
...
create table &libreria..BASE_EMAIL_TEFs AS
	SELECT	DISTINCT T1.RUT, 
		...
		AND inicio_correo not in (select inicio_correo from &libreria..INICIO_CORREO_INCORRECTOS_UNIF)
;
QUIT;
...
/*=========================================================================================*/
/*======	6.- Obtener Emails desde simulaciones en Sitio Público		===================*/
/*=========================================================================================*/

proc sql;
create table &libreria..SIMULACIONES_SP as
select *
from POLAVARR.SIMULACIONES_SP
where RUT < 99999999 AND RUT > 10000 AND RUT IS NOT MISSING
		AND dominio not in (select dominio from &libreria..DOMINIO_INCORRECTOS_UNIF) 
		and inicio_correo not in (select inicio_correo from &libreria..INICIO_CORREO_INCORRECTOS_UNIF) 
;quit;

PROC SQL;
CREATE INDEX rut ON &libreria..SIMULACIONES_SP (RUT);
QUIT;


/*=========================================================================================*/
/*=== 7.- Obtener Emails desde base histórica de simulaciones =============================*/
/*========  en Sitio Público (3 meses)=====================================================*/
/*=========================================================================================*/

proc sql;
create table &libreria..SIMULACIONES_SP_HIST as
select *
from POLAVARR.SIMULACIONES_SP_HIST
where RUT < 99999999 AND RUT > 10000 AND RUT IS NOT MISSING
		AND dominio not in (select dominio from &libreria..DOMINIO_INCORRECTOS_UNIF) 
		and inicio_correo not in (select inicio_correo from &libreria..INICIO_CORREO_INCORRECTOS_UNIF) 
;quit;
...
PROC SQL;
CREATE INDEX rut ON &libreria..SIMULACIONES_SP_HIST (RUT);
QUIT;


/*=========================================================================================*/
/*======	8.- Une Email de todos los Orígenes	actuales		===========================*/
/*======		QUE NO ESTÉN EN BOPERS O QUE TENGAN UNA FECHA MEJOR LOS AGREGA		=======*/
/*=========================================================================================*/
PROC SQL;
CREATE TABLE &libreria..R_EMAIL_UNIDOS AS					/* maxima secuencia 8 */
/*	SIMULACIONES_SP_HIST - SEQ 2 */
	SELECT
          t1.RUT, 
          t1.EMAIL length=50, 
		  t1.FECHA_ACT,
          t1.SEQUENCIA,
          t1.ORIGEN length=20
    FROM &libreria..SIMULACIONES_SP_HIST	t1
...
/*	SIMULACIONES_SP - SEQ 2 */
	SELECT
          ...
          t2.SEQUENCIA,
          t2.ORIGEN length=20
    FROM &libreria..SIMULACIONES_SP	t2
		LEFT JOIN &libreria..R_BOPERS_TOTALES_EMAIL tx on (t2.RUT=tx.rut) 
			where tx.RUT is null OR tx.ESTADO_ACT_VER = 1 AND t2.FECHA_ACT > tx.FECHA_ACT
union
/*	BOPERS	- SEQ ALEATORIA */
	SELECT	rut, 
          	...
          	FECHA_ACT,
           	SEQUENCIA,
            ORIGEN
    FROM &libreria..R_BOPERS_TOTALES_EMAIL
union
/*	FISA - SEQ 7 */
...
/*	RETAIL_COM 	- SEQ 1*/
   	SELECT	distinct t1.RUT, 
		  	...
		  	t1.sequencia,
		  	t1.ORIGEN
  	FROM PUBLICIN.BASE_EMAIL_COM_202302 t1 /*VA CON VARIABLE FECHA*/ 
		LEFT JOIN &libreria..R_BOPERS_TOTALES_EMAIL tx on (t1.RUT=tx.rut) 
			where tx.RUT is null OR tx.ESTADO_ACT_VER = 1 AND t1.FECHA_ACT > tx.FECHA_ACT
union 
...
union 
/*	QUIERO SER CLIENTE HB 	- SEQ 13 */
   	SELECT	distinct t10.RUT, 
          	t10.EMAIL length=50, 
          	t10.FECHA_ACT,
		  	t10.sequencia,
		  	t10.ORIGEN
  	FROM PUBLICIN.QSC_HB_E_NEW t10 
...
;
QUIT;
/*======	FIN 9.- Une Email de todos los Orígenes	actuales	=======================*/
/*=====================================================================================*/
...
/*=========================================================================================*/
/*======	10.- Cruce con Emails incorrectos y dominios incorrectos		===================*/
/*=========================================================================================*/
PROC SQL;
...
CREATE TABLE result.SP_OPEN_&fechax0. as 
	...
	where periodo >= &fechax0. and event_type='Open'
...
proc sql ;
 CREATE TABLE RESULT.SP_SENT_&fechax0. as 
	SELECT	t1.customer_id AS RUT,  
			COMPRESS(UPCASE(t1.email)) AS EMAIL length=50
FROM LIBCOMUN.output_email_&fechax0. t1 
	where periodo >= &fechax0. AND event_type= 'Sent'
;quit;
...
PROC SQL;
   ...
   SELECT rut, 
          ...
          MRC_EMAIL_APER_3M+ 
          MRC_EMAIL_SENT_3M+
		  APERTURAS_3M) AS NOTA,
		  APERTURAS_3M AS APERTURAS,
		  MRC_BOPERS,
		  ORI_CANAL
	FROM &libreria..NOTA_RANK_NEW_2020  WHERE MRC_HardBounce = 0 and MRC_SUPPRESSED <> -20
;
QUIT;

PROC SQL;
   CREATE TABLE MAXIMO_NOTA_VU_&VdateMES AS 
   SELECT rut, (MAX(NOTA)) AS NOTA
      FROM CALCULO_NOTA_VU_&VdateMES
      GROUP BY rut;
QUIT;
...
PROC SQL;
   CREATE TABLE BASE_TRABAJO_EMAIL AS 
   SELECT t1.RUT, 
          t1.EMAIL, 
          t1.MRC_BOPERS,
		  t1.APERTURAS,
