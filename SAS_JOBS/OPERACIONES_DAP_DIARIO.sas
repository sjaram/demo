...
call symput('fechai',"TO_DATE('"||input(put(intnx('month',today(),0,'begin'),DDMMYYD10. ),$10.)||"','DD-MM-YYYY')");
...
%DELETE_INCREM_PER_DIARIO(sas_ppff_operaciones_dap,raw,sasdata,0);

%INCLUDE"/sasdata/users_BI/RESULTADOS/oradiag_user_bi/AWS/AWS_EXPORT_INCREM_PER_DIARIO.sas";
%INCREM_PER_DIARIO(sas_ppff_operaciones_dap,PUBLICIN.OPERACIONES_DAP_&periodo,raw,sasdata,0);
...
TO = ("&DEST_2")
CC = ("&DEST_1", "&DEST_3")
SUBJECT = ("MAIL_AUTOM: Proceso OPERACIONES DAP DIARIO");
FILE OUTBOX;
 	PUT "Estimados:";
 	PUT "  Proceso OPERACIONES DAP DIARIO, ejecutado con fecha: &fechaeDVN";  
    PUT;
    PUT;
    PUT 'Proceso Vers. 02';
    PUT;
