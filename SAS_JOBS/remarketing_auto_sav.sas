...
date0x = put(intnx('day',today(),-1,'same'),date9.) ;
...
PROC SQL;
   CREATE TABLE WORK.SIMULACIONES_HOY_1 AS 
   SELECT t1.RUT, 
            (max(t1.fecha)) FORMAT=DATE9. AS fecha
...
PROC SQL;
   ...
   SELECT t1.RUT, 
          t1.fecha, 
          weekday(&fecha0) as dia_semana
      FROM WORK.SIMULACIONES_HOY_1 t1
;
QUIT;
 
PROC SQL;
   CREATE TABLE relacion_dia_semana_x AS 
   SELECT t1.RUT, t1.FECHA,
          case when t1.dia_semana =1 then &fecha0-2
               when t1.dia_semana <>1 then &fecha0 end format=date9. as dia_relativo
...
PROC SQL;
   ...
   SELECT t1.RUT,t1.fecha, t1.dia_relativo
      FROM WORK.relacion_dia_semana_x t1
      WHERE t1.fecha >= t1.dia_relativo;
QUIT;
...
PROC SQL;
   CREATE TABLE trx_all AS 
   SELECT t1.RUT,fecha format=date9. as fecha
      FROM JABURTOM.TRX_SAV_&fecha00 t1
       having fecha >= "&fecha3"d
       union 
   SELECT t1.RUT,fecha format=date9. as fecha
      FROM JABURTOM.TRX_SAV_&fecha11 t1
       having fecha >= "&fecha3"d
...
PROC SQL;
   CREATE TABLE SAV_TARGET_FINAL AS 
   SELECT distinct t1.RUT as customer_id, primer_nombre, email,fecha
...
PROC SQL;
   CREATE TABLE PPFF_SIMULACIONES_FINAL AS 
   SELECT distinct customer_id, primer_nombre, email, 'AV' AS PRODUCTO, fecha
     FROM AV_TARGET_FINAL
	 WHERE customer_id NOT IN (SELECT CUSTOMER_ID FROM SAV_TARGET_FINAL)

	 UNION

	SELECT distinct customer_id, primer_nombre, email, 'SAV' AS PRODUCTO, fecha
     FROM SAV_TARGET_FINAL
	 
	 UNION
	SELECT distinct customer_id, primer_nombre, email, 'SAV' AS PRODUCTO, "&fecha2"d as fecha
...
proc export data=WORK.SAV_TARGET_FINAL  
	OUTFILE="/sasdata/users94/ougarte/temp/SAV_TARGET_FINAL.CSV"	
	dbms=dlm;
	delimiter=';';
	PUTNAMES=yes;
RUN;
