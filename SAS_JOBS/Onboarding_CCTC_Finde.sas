...
/*==============================	Onboarding_CCTC_Finde		 	 ===============================*/
/* CONTROL DE VERSIONES
/* 2022-08-05 -- V05 -- David V.	-- Se agrega replace al export de archivo, para en caso de reejecutar actualice la salida.
/* 2022-08-01 -- V04 -- Sergio J.	-- Cambio urgente requ. Benja M. en variable fecha.
/* 2022-08-01 -- V03 -- David V.	-- Se agrega correo dbergoeingc@bancoripley.com
/* 2022-07-27 -- V02 -- David V.	-- Agrega código sas options validvarname=any;
...
finde=put(intnx('day',today(),-&n,'same'), yymmddn8.);
ayer=put(intnx('day',today(),-1,'same'), yymmddn8.);
periodo_act= input(put(intnx('month',today(),0,'same'),yymmn6. ),$10.);
periodo_ant= input(put(intnx('month',today(),-1,'same'),yymmn6. ),$10.);
periodo_ant2= input(put(intnx('month',today(),-2,'same'),yymmn6. ),$10.);

call symput("fec_proceso",fec_proceso);
Call symput("finde", finde);
...
where FECHA>=intnx('day',today(),-3) and FECHA<=intnx('day',today(),-1)
...
proc sql;
create table WORK.onboarding_ctacte_tc_2 as
select b.RUT,
b.rut_dv,
b.fecha_capta,
b.tipo_producto,
...
b.CV_Tenencia,
b.tc_tenencia
from base_call_max_seq a
left join  base_call_bopers b
on a.rut = b.rut
...
/*guardar en libreria*/
proc sql;
create table &libreria..ONB_CCTC_FINDE_&fec_proceso. as
select distinct
*
from WORK.onboarding_ctacte_tc_2
;quit;



/*para exportar archivo en correo*/
PROC EXPORT DATA=&libreria..ONB_CCTC_FINDE_&fec_proceso.
   OUTFILE="/sasdata/users94/user_bi/CC/ONB_CCTC_FINDE_&fec_proceso..csv" 
   DBMS=dlm replace; 
   delimiter=';'; 
   PUTNAMES=YES; 
RUN;
...
ATTACH	= "/sasdata/users94/user_bi/CC/ONB_CCTC_FINDE_&fec_proceso..csv"  
...
FILE OUTBOX;
 ...
 PUT ;
 PUT "	  	Se adjunta archivo: ONB_CCTC_FINDE_&fec_proceso..csv";
 PUT ;
 PUT ;
 PUT ;
 PUT 'Proceso Vers. 05'; 
