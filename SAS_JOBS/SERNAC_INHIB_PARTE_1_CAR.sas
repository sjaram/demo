...
/* Tablas que actualiza el proceso
 ...
 /* Archivos tomados y depositados en: /sasdata/users94/user_bi/SERNAC_NO_MOLESTAR/CAR/ 
 	- Tomados
...
x cd /sasdata/users94/user_bi/SERNAC_NO_MOLESTAR/CAR;
x ls -l bloqueos_mails.csv > actualizacion_bl_ml.txt;

data work.actualizacion;
   infile  "/sasdata/users94/user_bi/SERNAC_NO_MOLESTAR/CAR/actualizacion_bl_ml.txt" truncover;
    input variable  $2000.;   
run;
...
SUBJECT = ("WARNING, ARCHIVO PROCESO SERNAC DESACTUALIZADO - CAR");
FILE OUTBOX;
 PUT "Estimados, ¡Urgente!:";
 PUT " El proceso Sernac NO SE EJECUTA, &fecha_archivo ~= &fecha_hoy";
 put ; 
 put '    La Causa:'; 
 put '    Los archivos tomados desde: /sasdata/users94/user_bi/SERNAC_NO_MOLESTAR/CAR/'; 
 put ;
...
INFILE '/sasdata/users94/user_bi/SERNAC_NO_MOLESTAR/CAR/bloqueos_telefonos.csv'
	DELIMITER=';'
	FIRSTOBS=2
	MISSOVER
	DSD
...
DATA _null_;
 ...
 salida = COMPRESS('/sasdata/users94/user_bi/SERNAC_NO_MOLESTAR/CAR/telefonos_'||fecha||'.csv');
...
INFILE '/sasdata/users94/user_bi/SERNAC_NO_MOLESTAR/CAR/bloqueos_mails.csv'
...
DATA _null_;
 ...
 fecha = put(today(),yymmdd10.);
 salida = COMPRESS('/sasdata/users94/user_bi/SERNAC_NO_MOLESTAR/CAR/mails_'||fecha||'.csv');
...
/*CC = ("&DEST_2")*/
SUBJECT = ("MAIL_AUTOM: - PROCESO SERNAC - PARTE 1 - CAR");
FILE OUTBOX;
 PUT "Estimados:";
 put "    Proceso SERNAC, Parte 1, ejecutado con fecha: &fechaeDVN";  
 put ; 
 put '    Detalle:'; 
 put '    Archivos tomados y depositados en: /sasdata/users94/user_bi/SERNAC_NO_MOLESTAR/CAR/'; 
 put ;
