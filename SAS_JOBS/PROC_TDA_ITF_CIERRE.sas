...
proc sql;
	...
	select 
		...
		D.DV,
		/*variables Nuevas*/
		CASE WHEN D.SUCURSAL IS NOT MISSING THEN CATS(D.SUCURSAL,'.-',E.sucursal) ELSE 'SIN INFORMACION' END AS Nombre_SUC,
		CASE 
...
%DELETE_INCREM_PER_DIARIO(sas_mdpg_tda_itf,raw,sasdata,-1);

/*Exportación a AWS*/
%INCLUDE"/sasdata/users_BI/RESULTADOS/oradiag_user_bi/AWS/AWS_EXPORT_INCREM_PER_DIARIO.sas";
%INCREM_PER_DIARIO(sas_mdpg_tda_itf,&libreria..tda_itf_&periodo,raw,sasdata,-1);
...
data _null_;
	...
	SUBJECT = ("MAIL_AUTOM: Proceso PROC_TDA_ITF_CIERRE");
	FILE OUTBOX;
	PUT "Estimados:";
	PUT "		Proceso TDA_ITF_CIERRE, ejecutado con fecha: &fechaeDVN";
