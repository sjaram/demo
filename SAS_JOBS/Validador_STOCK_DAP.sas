...
/**/
proc sql;
create table work.VALIDACION_SROCK_DAP1
(
Fecha char(99), 
...
proc sql;
insert into WORK.VALIDACION_SROCK_DAP1  
values(
&Fecha_Proceso,
&Nombre_Proceso,
&Descripcion_Proceso,
...
PROC SQL;
   CREATE TABLE WORK.VALIDACION_SROCK_DAP AS 
   SELECT t1.Fecha, 
          t1.Nombre_Proceso, 
      ...
      FROM WORK.VALIDACION_SROCK_DAP1 t1;
QUIT;
...
%macro mensaje_correo(error) ;
	...
	SUBJECT= "Ejecución de Proceso STOCK_DAP"
	...
	ODS LISTING CLOSE;
	ODS HTML  path="/sasdata/users94/user_bi" file = "Validador_STOCK_DAP.lst" (URL=none) BODY=output STYLE=sasweb;
	TITLE JUSTIFY=left
	&temp_error;
	PROC PRINT DATA=VALIDACION_SROCK_DAP  NOOBS;
	RUN;
	ODS HTML CLOSE;
	ODS LISTING;
