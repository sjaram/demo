...
/* 2021-08-16 -- V1 -- Pia O -- SE CREA PRIMERA VERSION DEL PROCESO FORMULARIO   
...
proc import datafile='/sasdata/users94/user_bi/CONT_FORMU/resultados_formulario.csv'
	dbms=dlm out=capta_formu replace;
	delimiter=';';
	getnames=yes;
run;
 
proc sql;
create table CAPTA_FORMULARIO as 
select 
input(compress(substr('RUT Cliente'n,1,length('RUT Cliente'n)-2),'.'),best.) as rut,
'Fecha interacción'n,
Celular,
email 
from capta_formu
...
PROC SQL;
   CREATE TABLE FORMULARIO_A AS 
   SELECT T1.RUT,
		  input(put('Fecha interacción'n,yymmddn8.),best.) as FECHA_ACT,
		  'Fecha interacción'n AS FECHA_ORIGINAL,
		  COMPRESS(PUT(CELULAR-100000000* Floor(CELULAR/100000000),BEST.)) AS TELEFONO,
          upcase(t1.Email) 	AS EMAIL
      FROM work.CAPTA_FORMULARIO t1  					/*ACTUALIZAR NOMBRE AQUI Y EN EL WORK*/

;QUIT;

/*  FILTROS A EMAIL */
PROC SQL;
   CREATE TABLE &libreria..FORMULARIO_E_&VdatePeriodo AS  
   SELECT 	t1.RUT,
		  	t1.Email,
			T1.TELEFONO,
          	T1.FECHA_ACT,
			T1.FECHA_ORIGINAL
      FROM 	FORMULARIO_A t1
...
PROC SQL;
   CREATE TABLE &libreria..FORMULARIO_F_&VdatePeriodo AS 
...
proc sql;
create table FORMULARIO_E as
SELECT	t1.RUT, 
        upcase(t1.Email) as EMAIL, 
        t1.FECHA_ACT,
		13 AS SEQUENCIA,
		'FORMULARIO' AS ORIGEN
  	FROM &libreria..FORMULARIO_E_&VdatePeriodo t1 
	WHERE RUT > 100 AND RUT < 99999999 AND RUT IS NOT MISSING
;quit;


proc sql;
create table FORMULARIO_E_MAX AS
SELECT	DISTINCT RUT, 
        EMAIL, 
        MAX(T1.FECHA_ACT) AS FECHA_ACT,
        t1.SEQUENCIA,
        T1.ORIGEN
    FROM FORMULARIO_E t1
GROUP BY RUT
;quit;


proc sql;
create table FORMULARIO_E_UNI AS
SELECT	DISTINCT T2.RUT, 
        T1.EMAIL, 
		T2.FECHA_ACT,
        T2.SEQUENCIA,
        T2.ORIGEN
    FROM FORMULARIO_E T1 INNER JOIN FORMULARIO_E_MAX t2
...
proc sort data=FORMULARIO_E_UNI out=&libreria..FORMULARIO_E nodupkeys dupout=WORK.duplicados_RUT;			/* cambiado desde result */
by rut;
run;

PROC SQL;
CREATE INDEX rut ON &libreria..FORMULARIO_E (RUT);
QUIT;

proc sql;
create table &libreria..FORMULARIO_E_&VdatePeriodo AS 
	SELECT * FROM &libreria..FORMULARIO_E
;quit;
...
/*AGREGO SECUENCIA Y ORIGEN, ADEMÁS DE RANGO DE RUTS VÁLIDOS*/
proc sql;
create table FORMULARIO_F as
SELECT	t1.RUT, 
        t1.TELEFONO, 
		T1.FECHA_ORIGINAL,
		0 AS SEQUENCIA, /* ES TOMADA COMO NOTA O SCORE EN LOS FONOS */
		'FORMULARIO' AS ORIGEN
  	FROM &libreria..FORMULARIO_F_&VdatePeriodo t1
...
proc sql;
create table FORMULARIO_F_MAX AS
SELECT	DISTINCT T1.RUT, 
        T1.TELEFONO, 
		MAX(T1.FECHA_ORIGINAL) AS FECHA_ORIGINAL,
        t1.SEQUENCIA,
        T1.ORIGEN
    FROM FORMULARIO_F t1
GROUP BY RUT
;quit;


proc sql;
create table FORMULARIO_F_UNI AS
SELECT	DISTINCT T2.RUT, 
        t1.TELEFONO, 
        T1.FECHA_ORIGINAL as FECHA,
        T2.SEQUENCIA AS NOTA,
        T2.ORIGEN AS FUENTE
    FROM FORMULARIO_F T1 INNER JOIN FORMULARIO_F_MAX t2
...
/*	ACA QUDARÁN TODOS LOS EMAIL HASTA EL MINUTO	*/
proc sort data=FORMULARIO_F_UNI out=&libreria..FORMULARIO_F nodupkeys dupout=WORK.duplicados_RUT;			/* cambiado desde result */
by rut;
run;

PROC SQL;
CREATE INDEX rut ON FORMULARIO_F (RUT);
QUIT;

proc sql;
create table &libreria..FORMULARIO_F_&VdatePeriodo AS
	SELECT * FROM FORMULARIO_F
;quit;
...
TO = ("&DEST_3")
CC = ("&DEST_1", "&DEST_2")
SUBJECT = ("MAIL_AUTOM: Proceso CONTACT_FORMULARIO");
FILE OUTBOX;
 PUT "Estimados:";
 put "  Proceso CONTACT_FORMULARIO, ejecutado con fecha: &fechaeDVN";  
 put ; 
 put ; 
 PUT ;
 PUT ;
